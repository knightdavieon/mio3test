...
<body class="theme-black">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <!-- Input -->
            <a href="swho_receiving_items.php" class="btn btn-warning"><i class="material-icons">arrow_back</i> Back </a>
           ...
           if(isset($_POST['btn_commit'])){
              ...
              foreach($_POST['item_barcode'] as $key => $value){
                    ...
                    // Queries
                    $goods_query = mysqli_query($conn, "Select * from tbl_goods_receive where item_barcode IN ('" . $item_barcode . "')");
                    $goods_rows = mysqli_fetch_array($goods_query);
                    $item_quant = $goods_rows['item_quantity'] + $quantity;
                    $goods_update = mysqli_query($conn, "Update tbl_goods_receive set item_quantity = '" . $item_quant . "' where item_barcode = '" . $item_barcode . "'");
                    $transfer_history = mysqli_query($conn, "Insert into tbl_branch_transfer_history(employee_code, staff_name, transfer_to, transaction_number, item_barcode, item_name, cat, sub_cat, quant, price, remarks, ts_type, transfer_date, received_by, item_from)values('"
                    . $employee_code . "','" . $staff_name . "','" . $transfer_to . "','" . $transaction_number . "','" . $item_barcode . "','" . $item_name . "','" . $cat . "','" . $sub_category . "','" . $quantity . "','" . $price . "','" . $remarks . "','" . $ts_type . "','" . $date_received . "','" . $_SESSION['fullname'] . "','" . $current_branch . "')")or die("Error : " . mysqli_error($conn));
                    //$update_status = mysqli_query($conn, "Update tbl_branch_transfer_item_temp set receiving_status")
                    // Queries 
                    if($transfer_history === true){
                        $message = true;
                        
                    }
              }
              if($message){
                   $status_query = mysqli_query($conn, "Update tbl_branch_transfer_series set receiving_status = '" . "DONE" . "' where transaction_number = '" . $_GET['trans_num'] . "'");
                   echo '<script type="text/javascript"> alert("Committed Successfully"); window.location="swho_receiving_items.php"; </script>';
              }
           }
          ...
          ?>
           <form method="post" action="<?php echo $_SERVER['PHP_SELF'] . "?trans_num=" . $_GET['trans_num']; ?>" enctype="multipart/form-data">
                         <div class="body">
                                                 ...
                                                 <?php 
                                                    ...
                                                    while($rows = mysqli_fetch_array($result)){
                                                        ...
                                                        <tr>
                                                            ...
                                                            <input type="hidden" name="transaction_number[]" value="<?php echo $rows['transaction_number'] ?>">
                          ...
                          <a href="swho_receiving_items.php" class="btn btn-warning"><i class="material-icons">arrow_back</i> Back </a>
                          <button type="submit" class="btn btn-success" name="btn_commit" onclick="return confirm('Are you sure you want to commit this ? '); "><i class="material-icons">done_all</i> Approve </button>
                        </div>
