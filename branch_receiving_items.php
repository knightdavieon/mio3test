                                 ...
                                 table.dataTable thead th{
                                      ...
                                      <table class="table table-striped table-bordered" id="r_items">
                                            <thead>
                                             ...
                                             <tbody>
                                                 <?php 
                                                    $query = "Select * from tbl_transaction_series where branch_name = '" . $_SESSION['b_code'] . "'";
                                                    $result = mysqli_query($conn, $query);
                                                    while($rows = mysqli_fetch_array($result)){
                                                          if($rows['transfer_status'] == "PENDING"){
                                                              ?>
                                                        <tr>
                                                            <td><a href="branch_receiving_commit.php?trans_num=<?php echo $rows['transfer_number']; ?>"><?php echo $rows['transfer_number'] ?></a></td>
                                                            <td><?php echo $rows['employee_name'] ?></td>
                                                            <td><?php echo $rows['employee_code'] ?></td>
                                                            <td><?php echo $rows['date_received']; ?></td>
                                                            <td><label style="color:#e65c00;"><small><?php echo $rows['transfer_status']; ?></label></small></td>
                                                         </tr>
                                                    <?php
                                                          }
                                                    }
