...
<?php 
     if(isset($_GET['r_id'])){
        ...
        $info_rows = mysqli_fetch_array($info_result);
            ...
            $pdf->Ln(20);
            $pdf->Cell(40, 5, "Transaction Number : " . $info_rows['transaction_number'], 1);
            $pdf->Cell(150, 10, "Staff Name : " . $info_rows['person_transfered'],  1);
            $pdf->Ln(5);
            $pdf->Cell(40, 5, "Employee Code : " . $info_rows['employee_code'], 1, 0, 'RT');
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Cell($width, 5, "Transfer To : " . $info_rows['branch'], 2);
            $pdf->Ln();
            $pdf->Cell($width, 5, "Received Date : " . $info_rows['date_received'], 2);
            $pdf->Ln();
            
            $pdf->Cell(20, 5, "Barcode", 1, 0, 'C');
            ...
            while($rows = mysqli_fetch_array($result)){
                $pdf->Cell(20, 5, $rows['item_barcode'], 1, 0, 'C');
                $pdf->Cell(40, 5, $rows['item_name'], 1, 0, 'C');
                $pdf->Cell(20, 5, $rows['cat'], 1, 0, 'C');
                $pdf->Cell(20, 5, $rows['sub_category'], 1, 0, 'C');
                $pdf->Cell(30, 5, $rows['item_quant'], 1, 0, 'C');
                $pdf->Cell(20, 5, $rows['item_price'], 1, 0, 'C');
                $pdf->Cell(40, 5, $rows['remarks'], 1, 0, 'C');
                $pdf->Ln();
            }
            $pdf->Ln();
            $pdf->Ln();
